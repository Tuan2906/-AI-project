...
import danhSachnhaMay from './dataNhaMay.json'; // Import for nhà máy data
...
export default function TheoDoiBaiThi() {
    ...
    useEffect(() => {
        ...
        // Xử lý danhSachNhaMay (giống hệt danhSachNV, nhưng sửa key "phong ban" thành "phongBan")
        const initialNhaMay = danhSachnhaMay.map(nm => ({
            ...nm,
            phongBan: nm['phong ban'].replace(/Phòng |phong ban/gi, '').trim(), // Xử lý cả hai trường hợp key
            trangThai: 'ChuaLam'
        }));

        setDanhSachNV(initialNV as any);
        setDanhSachNhaMay(initialNhaMay as any);
        console.log('Initial danhSachNV:', initialNV);
        console.log('Initial danhSachNhaMay:', initialNhaMay);
    }, []);

    // Cập nhật dữ liệu từ API
    useEffect(() => {
        if (danhSachBaiThi && danhSachBaiThi.length > 0) {
            // Cập nhật danh sách nhân viên công ty
            ...
            // Cập nhật danh sách nhà máy (chỉ so khớp email)
            ...
            console.log('Updated danhSachNV:', updatedNV);
            console.log('Updated danhSachNhaMay:', updatedNhaMay);
            console.log('API Data:', danhSachBaiThi);
        }
    ...
    // Lọc dữ liệu theo trạng thái và tìm kiếm tên
    const filteredNV = useMemo(() => {
        ...
        console.log('Filtered danhSachNV:', filtered);
        return filtered;
    ...
    const filteredNhaMay = useMemo(() => {
        ...
        console.log('Filtered danhSachNhaMay:', filtered);
        return filtered;
